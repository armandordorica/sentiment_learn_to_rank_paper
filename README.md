...
| 2026-07-17 | **Leakage-safe pooled five-stock fine-tuning** — extended the RavenPack training pipeline and FastAPI Section 5 to select multiple tickers and train them jointly in one pass, added a five-stock preparation utility, temporal split/duplicate leakage audits, pooled coverage details, and per-ticker held-out test metrics. Fine-tune resume coverage now includes the pooled configuration. *Decision:* pool all selected tickers before fitting rather than adapting sequentially. *Why:* joint training reduces catastrophic forgetting while ticker-aware temporal boundaries and overlap checks prevent train/validation/test leakage. |
...
| 2026-08-01 | **One-stock story + RavenPack relevance** — Data Explorer **1D** “Read story + relevance” now opens the Refinitiv full story and looks up RavenPack `relevance_score` for the same ticker near that timestamp (`src/sentiment_ltr/data/ravenpack_match.py`): prefers a WRDS day pull with headlines (cached under `data/raw/data_explorer_ravenpack_days/`), falls back to nearby batch-cache events when headlines were omitted. *Decision:* join on soft headline match inside a time window rather than inventing a Refinitiv relevance field. *Why:* Refinitiv has the readable wire story; RavenPack has entity relevance — the inspection UX needs both in one click. |
| 2026-08-01 | **FastAPI parity for remaining Streamlit gaps** — ported Section **5·1–5·7** AAPL before/after analysis to `/finetune` (lazy HTMX scoring via `webapp/api/finetune_analysis.py`); PhraseBank **3B** reproduction recipe + **3F** W&B project/run links; Data Explorer **1E** RavenPack inspect panel; 1D headline list collapsible top-N/filter/sort; 1B/1C deduped so price chart appears once. *Decision:* FastAPI is the primary UI; Streamlit remains fallback only. *Why:* close the last subsection gaps called out in `docs/fastapi_migration_plan.md` so research work can stay on one stack. |
| 2026-08-01 | **RavenPack headlines always pulled; no timestamp relevance** — WRDS RavenPack queries always select `headline`, and by default also `event_text` when present (`include_text=True`). Data Explorer prefers rich exports / joins titles onto legacy batch caches. Refinitiv story relevance uses headline soft-match only — removed assigning nearby same-day RavenPack scores by timestamp. *Decision:* title (+ event snippet) match over time proximity. *Why:* many unrelated AAPL events share a day; without titles soft-matching Refinitiv↔RavenPack is impossible, and event_text is the only RavenPack “body” available. |
...
| 2026-08-02 | **Full-story pull progress bar + pause/halt + storage** — live HTMX status shows % this-run / overall, stories/min, ETA, bytes on disk / added / projected finish size; **Pause** and **Halt** write `_pull_control.json` so the worker stops after the current story (resume via Load selected); a per-ticker storage table sums body sizes under `data_explorer_full_stories/`. *Decision:* cooperative control file (not kill -9) so partial writes stay consistent. *Why:* ~27k AAPL bodies need tqdm-like feedback and a safe stop, plus disk sizing before scaling to more tickers. |
| 2026-08-02 | **Per-day story pull progress grid** — each ticker’s pull window renders a GitHub-style calendar (one square/day): gray=not done, yellow=in progress, green=all bodies loaded, blue=no headlines that day, red=attempts finished with failures; green/red use shade 1–5 by count; hover shows day detail (tried/retrieved/failed/pending). Built from headline parquet + on-disk digests + manifest failures; refreshes with the live status poll (attach via **Check cache only** without starting a second job). *Decision:* day rollup over story-level fetch, not a separate daily API. *Why:* make multi-year coverage scannable at a glance while a long AAPL pull runs. |
| 2026-08-02 | **1A grouped per-field Load checkboxes** — Data Explorer sources are grouped by API (Refinitiv / WRDS / Yahoo / RavenPack) with Unselect-all plus per-group Select/Unselect; each field is its own checkbox (`pull_fields=product_id::field`) mapped back to product ids server-side. Inventory table matches the same grouping. *Decision:* field tokens over product-only toggles. *Why:* load exactly the columns you need and skip whole providers without hunting a flat list. |
| 2026-08-02 | **Thompson-sampling rate-limit pacer** — full-story pulls now default to a Beta-Bernoulli multi-armed bandit over log-spaced wait arms (`AdaptivePacer` policy `thompson`); failed 429 cooldowns debit an arm, successful recoveries credit it, and state persists in `_pacer_bandit.json` across runs. AIMD remains available via `--pacer aimd`. UI shows wait / learned prefer while rate-limited. *Decision:* bandit RL over fixed exponential backoff so the pull learns a sustainable cooldown instead of blindly doubling to the ceiling. *Why:* LSEG 429s were stalling AAPL body pulls at 0 fetched while sleep climbed to 120s with no memory across retries. |
| 2026-08-02 | **Overnight cool-off + resume loop** — after every 6 consecutive 429s the pull takes a 15‑min `cooling_off` pause (interruptible via Halt), and `--overnight` restarts sessions every 20 min until bodies are complete. Webapp story jobs launch with these flags; CLI docs use `caffeinate -is` so idle sleep does not pause the worker. *Decision:* long cool-offs + session restarts over hammering the first story all night. *Why:* go-to-bed pulls need to wait for Refinitiv quota to reset, then keep going without babysitting. |
| 2026-08-04 | **Workspace daily story quota** — LSEG desktop news is ~5 req/s and ~10k req/day across all apps on the same Workspace; `get_story` is one request per body. Pulls now default to `--max-per-day 9000` (counts today's successful fetches across tickers) and wait until local midnight instead of 429-looping. AAPL local-day counts hit 9,997 / 9,171 and MSFT's 830 made 10,001 on 2026-08-04. Webapp min-sleep raised to 0.3s (~3 req/s). *Why:* overnight MSFT 429s were the daily cap after AAPL, not a dead login. |
| 2026-08-04 | **Hourly story-quota cron** — `scripts/run_daily_story_quota.py` spends remaining daily `get_story` budget (skip if a pull is live or cap is used), walking `app_data/story_pull_queue.txt`. Install via `scripts/install_story_quota_cron.sh` (`5 * * * *` + `caffeinate`). Cron is a watchdog; the in-process daily cap still pauses at 9k. *Why:* use the 10k/day window every calendar day without babysitting or dual jobs. |
| 2026-08-05 | **Dual quota windows** — scheduler + in-process waiter take the tighter of local calendar-day and rolling-24h 9k caps, then sleep until the later of next midnight vs oldest-in-window+24h. Cron logs both counters; Data Explorer shows which window is binding. Empirical Aug 4 logs hit exactly 10,001 on the local calendar day but also went past 10k inside 24h, so docs stay ambiguous. *Why:* do not 429-hammer at midnight if LSEG’s cap is rolling, and do not sit idle all day if only yesterday’s evening burst is still in the 24h window. |
| 2026-08-05 | **News-threshold universe + headline/story pipeline** — wrote `app_data/ravenpack_news_threshold_universe.csv` (RavenPack ≥1 article/week from the 2B top-1k cache), synced `story_pull_queue.txt` to every ticker that already has usable Refinitiv headlines, and added `scripts/backfill_refinitiv_headlines.py` (skips while a story job holds the shared news quota). 2B now shows this set separately from “complete” provider status. *Decision:* keep one `get_story` worker; expand headlines only when stories are idle. *Why:* maximize data for the ~500 paper-like names without parallel LSEG news jobs fighting MSFT. |
| 2026-08-05 | **Daily story cap 9,800** — raised `--max-per-day` default from 9,000 → 9,800 (still under LSEG’s ~10k Workspace ceiling). *Decision:* keep a small 200-call cushion rather than ride 10k. *Why:* batch prices/news are already cached; the only live news traffic is this one story worker. |
| 2026-08-05 | **Story-quota settings in Many stocks (2F)** — `max_per_day`, min-sleep, and hourly cron minute live in `app_data/story_quota_settings.json`, editable under Batch Pipeline **2F · Story quota automation** (applies crontab install/update/remove). CLI + cron + Data Explorer story launches read the same file. *Why:* tune the shared multi-ticker Workspace budget without editing shell/cron by hand. |
| 2026-08-05 | **2F status-first UI + full universe queue** — story-quota panel is read-only by default (Edit settings to change); shows live pull status, next cron check in UTC and Eastern, and the full news-threshold pull queue (~517 unique tickers) with Running/Next/Ready/Needs-headlines/Done. Queue file lists the whole ≥1/week set; cron still skips names lacking usable Refinitiv headlines. *Decision:* display defaults, don’t invite constant retuning; queue the full universe even before headline backfill. *Why:* make automation inspectable end-to-end for the ~500-name paper set. |
| 2026-08-06 | **Mac Mini migration guide** — added root `migration.md` with AirDrop-vs-git+rsync steps, conda recreate, Workspace, cron, uvicorn/SSH, and a Cursor paste prompt for the destination machine. *Why:* move code+data to an always-on Mini without losing gitignored caches or assuming the conda env copies. |
| 2026-08-06 | **Workspace + credential verification section** — extended `migration.md` with LSEG Workspace install, `.env` checks, `test_refinitiv_connection.py`, WRDS smoke query, and AirDrop transfer size/count checks. *Why:* Mini setup needs an explicit pass/fail path for desktop auth and copied caches. |
| 2026-08-06 | **Cloudflare domain hosting for Mac Mini** — added `docs/cloudflare_hosting.md` (named Tunnel → `127.0.0.1:8001`, DNS CNAME, LaunchAgent notes, Cloudflare Access) and linked it from `migration.md`. *Decision:* Tunnel over router port-forward so the home Mini stays off the public WAN. *Why:* serve the FastAPI webapp on a bought domain without exposing LAN ports; Access recommended because the app has no built-in auth. |
...
3. **Expand Refinitiv coverage on the news-threshold set** — leave MSFT (and the story queue) on the 9k/day body pull; when that job is idle run `scripts/backfill_refinitiv_headlines.py` so more of the ~500 names get `storyId`s, then let cron walk the updated queue. RavenPack articles + WRDS prices are already the bulk modeling corpus.
