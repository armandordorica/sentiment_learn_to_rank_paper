...
| Exclude stocks with &lt;1 news article/week → ~512 names | **Partial** | RavenPack proxy filter is written to `app_data/ravenpack_news_threshold_universe.csv` (rebuild: `scripts/build_news_threshold_universe.py --sync-story-queue`). 2B still reports top-1k complete/partial (WRDS+Yahoo+RP+Refinitiv prices). Refinitiv headlines exist for only a few dozen names; full bodies are AAPL ≈ done and MSFT in the 9k/day queue. Headline backfill: `scripts/backfill_refinitiv_headlines.py` (skips while a story job is live). | Streamlit **2 · Batch Pipeline** (section **2B · Cached data snapshot**); FastAPI [`/batch`](http://localhost:8001/batch) |
