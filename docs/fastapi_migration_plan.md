...
|---|-----|--------|----------|-------|
| 1 | **Data Explorer** | ✅ | 1A–1F | Ported at `/data-explorer`. **1D** collapsible top-N/filter/sort headlines + story↔RavenPack relevance; **1E** collapsible RavenPack inspect panel (relevance/taxonomy). |
...
| 3 | **PhraseBank HF Baseline** | ✅ | 3A Model & training · 3B Reproduction recipe · 3C Performance metrics · 3D Dataset dashboard · 3F W&B experiment tracking | Ported at `/phrasebank`: 3A inventory + LR/batch, **3B** `phrasebank_baseline_recipe()` tables with GitHub pointers, 3C/3D charts + train-split eval, **3F** project + imported 3-epoch/1-epoch W&B links. |
...
| 5 | **RavenPack Fine-Tuning ⭐** | ✅ | 5·1–5·7 AAPL before/after · **5·8** Train 1/5/N · OOD baskets | `/finetune` now includes **5·1** split overview, **5·2** tokenization, lazy HTMX **5·3–5·6** before/after (macro-F1, per-class, prevalence, mismatch samples), **5·7** hyperparams/provenance (`webapp/api/finetune_analysis.py`), plus existing 5·8 training and three-basket OOD benchmark. |
| 6 | **Sentiment Lab** | ✅ | 6A News data coverage · 6B Compute device · 6C Financial PhraseBank dataset · 6D RavenPack articles browser · 6E Live inference (score a headline) · background PhraseBank training | Ported at `/sentiment-lab` with Jinja2 + HTMX, reusing the shared model implementation. |
| 7 | **Paper Validation (2003–2014)** | ✅ | 7A Universe summary · 7B Top 20 by volume · 7C Monthly volume over time · 7D Monthly prices | Ported at `/paper-validation` with the bundled CRSP universe, validation checks, Plotly charts, top-20 table, and HTMX ticker price selector. |
...
| Background job polling pattern (batch pipeline tab) decided | ✅ | Both long-running jobs now run in **subprocesses**, not the web thread: Tab 2's batch runner (`scripts/run_batch_pipeline.py` → `batch.pid`/`batch_status.json`, HTMX polls `/batch/status` every 5s) and 5·8 fine-tuning (`scripts/finetune_worker.py` → per-job status JSON in `data/models/_finetune_runs/`; `webapp/jobs.py` `JobManager` thread spawns + polls it, HTMX polls `/finetune/train/{id}/status` every 2s). 5·8 was originally in-thread but HF `Trainer`+`accelerate`'s process-global state crashes (`AcceleratorState … distributed_type`) in a background thread — the subprocess gives a clean main-thread process and isolates a training crash from the server. |
